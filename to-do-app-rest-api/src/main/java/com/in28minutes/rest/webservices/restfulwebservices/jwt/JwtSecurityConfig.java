...
//    }
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
		...
		return httpSecurity
				...
				.oauth2ResourceServer(oauth2 -> oauth2.jwt(withDefaults())).httpBasic(customizer -> {
					customizer.authenticationEntryPoint(customAuthenticationEntryPoint());
					System.out.println("CustomAuthenticationEntryPoint set");
				}).headers(header -> header.frameOptions(HeadersConfigurer.FrameOptionsConfig::sameOrigin)).build();
	}
	...
	public UserDetailsService userDetailsService(PasswordEncoder passwordEncoder) {
		UserDetails user = User.withUsername("in28minutes2").password(passwordEncoder.encode("dummy")) // Encode
																										// password
				.authorities("read").roles("USER").build();
		UserDetails user2 = User.withUsername("raash").password(passwordEncoder.encode("raash")) // Encode password
				.authorities("*").roles("Admin").build();
